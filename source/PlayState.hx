...
class PlayState extends MusicBeatState
{
	...
	var glitchedStage:FlxSprite;
	var glitchedHex:Character;
	var glitchedBF:Boyfriend;

	var unGlitchedStageFront:FlxSprite;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case  'dunk':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dunk/dia'));
			case  'ram':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ram/dia'));
			case  'hello-world':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hello-world/dia'));
			case  'glitcher':
				dialogue = CoolUtil.coolTextFile(Paths.txt('glitcher/dia'));
		}
		...
		else if (SONG.song.toLowerCase() == 'glitcher')
		{
			defaultCamZoom = 0.9;
			curStage = 'bsodBasket';
			unGlitchedBg = new FlxSprite(-600, -200).loadGraphic(Paths.image('glitcher/stageback', 'hex'));
			unGlitchedBg.antialiasing = true;
			unGlitchedBg.scrollFactor.set(0.9, 0.9);
			unGlitchedBg.active = false;
			add(unGlitchedBg);

			unGlitchedStageFront = new FlxSprite(-650, 600).loadGraphic(Paths.image('glitcher/stagefront', 'hex'));
			unGlitchedStageFront.setGraphicSize(Std.int(unGlitchedStageFront.width * 1.1));
			unGlitchedStageFront.updateHitbox();
			unGlitchedStageFront.antialiasing = true;
			unGlitchedStageFront.scrollFactor.set(0.9, 0.9);
			unGlitchedStageFront.active = false;
			add(unGlitchedStageFront);

            glitchedBF = new Boyfriend(770, 450, 'bf-wire');
			glitchedHex = new Character(100, 100, 'hex-wire');
			glitchedStage = new FlxSprite(-600, -200).loadGraphic(Paths.image('WIRE/WIREStageBack', 'hex'));
			glitchedStage.antialiasing = true;
			glitchedStage.scrollFactor.set(0.9, 0.9);
			glitchedStage.active = false;
			
			glitchedBF.alpha = 0;
			glitchedHex.alpha = 0;
			glitchedStage.alpha = 0;
		...
		var diagHex:HEXDialogueBox = new HEXDialogueBox(false, dialogue);
		diagHex.scrollFactor.set();
		diagHex.finishThing = startCountdown;
	...
	function hexIntro(?diag:HEXDialogueBox):Void
	{
		if (diag != null)
		{
			inCutscene = true;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (curStage == 'bsodBasket')
			{
				glitchedHex.playAnim('idle');
			    glitchedBF.playAnim('idle');
			}
	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			...
			if (curStage == 'bsodBasket')
			{
				if (glitchedBF.animation.curAnim.name.startsWith('sing') && !glitchedBF.animation.curAnim.name.endsWith('miss'))
				{
					glitchedBF.playAnim('idle');
				}
			}
	...
	function noteMiss(direction:Int = 1):Void
	{
		...
		if (curStage == 'bsodBasket' && !glitchedBF.stunned)
		{
			switch (direction)
			{
				case 0:
					glitchedBF.playAnim('singLEFTmiss', true);
				case 1:
					glitchedBF.playAnim('singDOWNmiss', true);
				case 2:
					glitchedBF.playAnim('singUPmiss', true);
				case 3:
					glitchedBF.playAnim('singRIGHTmiss', true);
			}
		}
		...
		function goodNoteHit(note:Note):Void
			{
				...
				if (curStage == 'bsodBasket')
				{
					...
					{
						case 2:
							glitchedBF.playAnim('singUP', true);
						case 3:
							glitchedBF.playAnim('singRIGHT', true);
						case 1:
							glitchedBF.playAnim('singDOWN', true);
						case 0:
							glitchedBF.playAnim('singLEFT', true);
					}
				}
	...
	override function stepHit()
	{
		...
		if(curSong == "Glitcher")
		{
			...
			{
				case 576, 1088:
					FlxTween.tween(glitchedStage, {alpha: 1}, 0.15, {ease: FlxEase.linear});
					FlxTween.tween(unGlitchedStageFront, {alpha: 0}, 0.15, {ease: FlxEase.linear});
					...
					FlxTween.tween(glitchedBF, {alpha: 1}, 0.15, {ease: FlxEase.linear});
				case 832, 1345:
					FlxTween.tween(glitchedStage, {alpha: 0}, 0.15, {ease: FlxEase.linear});
					FlxTween.tween(unGlitchedStageFront, {alpha: 1}, 0.15, {ease: FlxEase.linear});
					...
					FlxTween.tween(glitchedBF, {alpha: 0}, 0.15, {ease: FlxEase.linear});
	        }
        }
	}
	...
	if (curStage == 'bsodBasket')
	{
		if (glitchedBF.animation.curAnim.name.startsWith('sing') && !glitchedBF.animation.curAnim.name.endsWith('miss') && (glitchedBF.animation.curAnim.curFrame >= 10 || glitchedBF.animation.curAnim.finished))
	    {
			glitchedBF.playAnim('idle');
		}
	}
